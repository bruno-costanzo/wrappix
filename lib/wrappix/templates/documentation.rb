module Wrappix
  module Templates
    class Documentation
      ...
      def self.render_endpoint_docs(endpoint, resource_name, singular_name, module_name, base_url)
        name = endpoint["name"]
        ...
        method_params << "body" if ["POST", "PUT", "PATCH"].include?(method)
        ...
        if path_params.empty? && !endpoint["params"] && !["POST", "PUT", "PATCH"].include?(method)
      ...
      def self.param_description(param)
        ...
        when /^(\w+)_id$/
          "The identifier of the #{$1}."
        else
          "Description not available."
        end
      end

      def self.response_example(name, resource_name, is_collection)
        case name

...
class ConfigurationTemplateTest < Minitest::Test
  def test_default_values
    ...
    assert_match(/@base_url = "https:\/\/api\.example\.com"/, content)
    assert_match(/@timeout = 30/, content)
    assert_match(/"Content-Type" => "application\/json"/, content)
    assert_match(/"Accept" => "application\/json"/, content)
  end
  ...
  def test_custom_base_url
    ...
    assert_match(/@base_url = "https:\/\/custom-api\.com\/v2"/, content)
  end
  ...
  def test_oauth_config
    ...
    assert_match(/@token_url = "https:\/\/auth\.custom-api\.com\/token"/, content)
